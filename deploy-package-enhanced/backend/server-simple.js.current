...
// Real-time metrics tracking middleware
...
app.get('/api/status', async (req, res) => {
  try {
    // Try enhanced version with real-time metrics first
    try {
      ...
      console.error('⚠️  Enhanced status failed (using fallback):', enhancedError.message)
    }
...
app.get('/api/status/api-status', async (req, res) => {
  try {
    ...
    try {
      ...
      console.error('⚠️  Enhanced API status failed (using fallback):', enhancedError.message)
    }
...
app.get('/api/status/analytics', async (req, res) => {
  try {
    ...
    try {
      ...
      console.error('⚠️  Enhanced analytics failed (using fallback):', enhancedError.message)
    }
...
app.listen(PORT, () => {
  ...
  initializeMetrics().then(() => {
    console.log('✅ Real-time metrics tracking initialized')
  }).catch(err => {
    console.error('⚠️  Metrics initialization failed (will use fallback):', err.message)
  })
