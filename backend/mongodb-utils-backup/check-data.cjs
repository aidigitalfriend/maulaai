...
async function checkData() {
  ...
  const chats = await db.collection('chatinteractions').find({}).limit(3).toArray();
  console.log(JSON.stringify(chats, null, 2));

  console.log('\n=== PAGEVIEWS SAMPLE ===');
  const pageviews = await db.collection('pageviews').find({}).limit(3).toArray();
  console.log(JSON.stringify(pageviews, null, 2));

  console.log('\n=== APIUSAGES SAMPLE ===');
  const apiusages = await db.collection('apiusages').find({}).limit(3).toArray();
