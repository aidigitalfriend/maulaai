...
async function traceUserJourney() {
  try {
    ...
    const userSubscriptions = await subsCol.find({ 
    ...
    if (userSubscriptions.length > 0) {
      ...
      console.log('  ✓ user:', sample.user ? 'ObjectId (correct)' : '❌ MISSING or wrong type');
      ...
      console.log('  ✓ billing.interval:', sample.billing?.interval || '❌ MISSING');
      console.log('  ✓ billing.amount:', sample.billing?.amount || '❌ MISSING');
      console.log('  ✓ billing.currentPeriodEnd:', sample.billing?.currentPeriodEnd || '❌ MISSING');
      console.log('  ✓ stripeSubscriptionId:', sample.stripeSubscriptionId || '❌ MISSING');
      ...
      const cancelled = userSubscriptions.filter(s => s.status === 'cancelled').length;
      ...
      console.log('\nDuration Breakdown:');
      const daily = userSubscriptions.filter(s => s.billing?.interval === 'day' || s.plan === 'daily').length;
      const weekly = userSubscriptions.filter(s => s.billing?.interval === 'week' || s.plan === 'weekly').length;
      const monthly = userSubscriptions.filter(s => s.billing?.interval === 'month' || s.plan === 'monthly').length;
    ...
    userSubscriptions.forEach(sub => {
      ...
      if (!agentCounts[agentId]) {
        agentCounts[agentId] = { total: 0, active: 0, cancelled: 0, expired: 0 };
      }
    ...
    Object.entries(agentCounts).forEach(([agentId, counts]) => {
      if (counts.total > 1) {
        hasRepurchases = true;
        console.log(`  ${agentId}: ${counts.total} times (${counts.active} active, ${counts.cancelled} cancelled, ${counts.expired} expired)`);
      }
    });
    ...
    if (userSecurity) {
      ...
      console.log('  ✓ twoFactorSecret:', userSecurity.twoFactorSecret ? 'present' : 'N/A');
    ...
    console.log('  Backend stores: userId (string), twoFactorEnabled (boolean)');
    console.log('  ⚠️ WARNING: userId is STRING, not ObjectId (inconsistent with subscriptions)');
    ...
    const platformSubs = await subsCol.find({ 
      user: new ObjectId(testUserId),
    ...
    if (platformSubs.length > 0) {
      ...
      console.log('  ✓ plan:', sample.plan ? 'ObjectId (references plans collection)' : '❌ MISSING');
      console.log('  ✓ status:', sample.status);
      console.log('  ✓ billing:', sample.billing ? 'present' : '❌ MISSING');
      console.log('  ⚠️ agentId:', sample.agentId ? 'present' : 'N/A (correct for platform plan)');
    } else {
      console.log('  ℹ️ No platform subscriptions found (only agent subscriptions)');
    }
    ...
    const userInvoices = await invoicesCol.find({ userId: testUserId }).toArray();
    const userPayments = await paymentsCol.find({ userId: testUserId }).toArray();
    const userBillings = await billingsCol.find({ userId: testUserId }).toArray();
    
    console.log('Collection: invoices');
    console.log(`  Found: ${userInvoices.length} invoice(s)`);
    console.log('  ❌ PROBLEM: Collection is EMPTY - invoices not being created!');
    
    console.log('\nCollection: payments');
    console.log(`  Found: ${userPayments.length} payment(s)`);
    console.log('  ❌ PROBLEM: Collection is EMPTY - payments not being tracked!');
    
    console.log('\nCollection: billings');
    console.log(`  Found: ${userBillings.length} billing record(s)`);
    console.log('  ❌ PROBLEM: Collection is EMPTY - billing history not saved!');
    
    console.log('\n⚠️ CRITICAL FINDING:');
    console.log('  Payments go through Stripe, subscriptions are created,');
    console.log('  BUT invoice/payment/billing records are NOT being saved to database!');
    ...
    console.log('  • subscriptions collection - field names match frontend/backend');
    ...
    console.log('  5. Standardize field naming: use "user" everywhere (not "userId")');
