...
// Core AI routing for real-time assistant responses
...
// Single shared multimodal AI service instance for AI Lab routes
...
router.post(
  ...
  handleAsync(async (req, res) => {
    ...
    if (!userId) {
      return res
        .status(401)
        .json(createResponse(false, null, 'Authentication required'));
    }
    ...
    try {
      const writingData = {
        ...
        content: {
          ...req.body.content,
          wordCount: req.body.content?.text
            ? req.body.content.text.split(/\\s+/).length
            : 0,
          characterCount: req.body.content?.text
            ? req.body.content.text.length
            : 0,
          paragraphCount: req.body.content?.text
            ? req.body.content.text.split('\\n\\n').length
            : 0,
        },
      };
      ...
      if (writingData.content.wordCount) {
        writingData.content.readingTime = Math.ceil(
          writingData.content.wordCount / 200
        ); // 200 WPM
      }
      ...
      res
        ...
        .json(
          createResponse(
            true,
            writing,
            'Creative writing project created successfully'
          )
        );
    } catch (error) {
...
router.get(
  ...
  handleAsync(async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json(createResponse(false, null, 'Failed to fetch creative writings'));
    }
  })
);
...
router.put(
  ...
  handleAsync(async (req, res) => {
    ...
    if (!validateObjectId(id)) {
      return res
        .status(400)
        .json(createResponse(false, null, 'Invalid writing ID'));
    }

    try {
      const writing = await CreativeWriting.findOne({
        _id: id,
        $or: [{ userId }, { 'collaboration.contributors.userId': userId }],
      });

      if (!writing) {
        return res
          .status(404)
          .json(createResponse(false, null, 'Creative writing not found'));
      }

      // Create new version if collaborative
      if (writing.collaboration.isCollaborative && req.body.content?.text) {
        await writing.addVersion(
          req.body.content.text,
          req.body.changes || 'Content update',
          userId
        );
      }
      ...
      if (req.body.content?.text) {
        ...
        writing.content.paragraphCount =
          req.body.content.text.split('\\n\\n').length;
        writing.content.readingTime = Math.ceil(
          writing.content.wordCount / 200
        );
      }
      ...
      res.json(
        createResponse(true, writing, 'Creative writing updated successfully')
      );
    } catch (error) {
      ...
      res
        ...
        .json(
          createResponse(
            true,
            assistant,
            'Smart assistant created successfully'
          )
        );
    } catch (error) {
...
router.get(
  ...
  handleAsync(async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json(createResponse(false, null, 'Failed to fetch smart assistants'));
    }
  })
);

// Chat with smart assistant (real-time AI using configured providers)
router.post(
  '/smart-assistant/:id/chat',
  handleAsync(async (req, res) => {
    const { id } = req.params;
    ...
    if (!validateObjectId(id)) {
      return res
        .status(400)
        .json(createResponse(false, null, 'Invalid assistant ID'));
    }

    if (!message || typeof message !== 'string') {
      return res
        .status(400)
        .json(createResponse(false, null, 'Message is required'));
    }
    ...
    try {
      ...
      if (!assistant) {
        return res
          .status(404)
          .json(createResponse(false, null, 'Smart assistant not found'));
      }

      // Add user message to conversation history
      await assistant.addMessage(conversationId, 'user', message);

      // Use a neutral personality for Smart Assistants so they are not tied
      // to a specific character agent (e.g. comedy-king, julie-girlfriend).
      const agentIdForRouting = 'random';

      // Derive basic AI configuration from assistant settings when available
      const aiConfig = {
        provider: 'auto',
        temperature: assistant.configuration?.temperature ?? 0.7,
        maxTokens: assistant.configuration?.maxTokens ?? 1500,
      };

      // Generate real-time AI response using the shared multimodal service
      const aiResult = await multiModalAIService.getChatResponse(
        message,
        agentIdForRouting,
        aiConfig
      );
      ...
      await assistant.addMessage(conversationId, 'assistant', aiResponse, {
        processingTime: aiResult.latency ?? 0,
        confidence: 0.9,
      });
      ...
      res
        .status(500)
        .json(createResponse(false, null, 'Failed to process message'));
    }
  })
);
...
router.post(
  ...
  handleAsync(async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json(createResponse(false, null, 'Failed to update memory'));
    }
  })
);
...
router.post(
  ...
  handleAsync(async (req, res) => {
    ...
    try {
      ...
      res
        .status(201)
        .json(createResponse(true, vr, 'VR experience created successfully'));
    } catch (error) {
...
router.get(
  ...
  handleAsync(async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json(createResponse(false, null, 'Failed to fetch VR experiences'));
    }
  })
);
...
router.post(
  ...
  handleAsync(async (req, res) => {
    ...
    if (!validateObjectId(id)) {
      return res
        .status(400)
        .json(createResponse(false, null, 'Invalid VR experience ID'));
    }
    ...
    try {
      ...
      if (!vr) {
        return res
          .status(404)
          .json(createResponse(false, null, 'VR experience not found'));
      }
      ...
      res
        .status(500)
        .json(createResponse(false, null, 'Failed to start VR session'));
    }
  })
);
...
// Rate VR experience
router.post(
  '/virtual-reality/:id/rate',
  handleAsync(async (req, res) => {
    const { id } = req.params;
    const { overall, immersion, comfort, content, performance, comment } =
      req.body;
    const userId = req.user?.id;
      ...
      res
        ...
        .json(
          createResponse(
            true,
            learning,
            'Language learning program created successfully'
          )
        );
    } catch (error) {
      ...
      res
        ...
        .json(
          createResponse(
            false,
            null,
            'Failed to fetch language learning programs'
          )
        );
    }
  })
...
// Add vocabulary word
router.post(
  '/language-learning/:id/vocabulary',
  handleAsync(async (req, res) => {
    const { id } = req.params;
    ...
    if (!validateObjectId(id)) {
      return res
        .status(400)
        .json(createResponse(false, null, 'Invalid learning program ID'));
    }
    ...
    try {
      ...
      if (!learning) {
        return res
          .status(404)
          .json(
            createResponse(false, null, 'Language learning program not found')
          );
      }

      await learning.addVocabularyWord(
        word,
        translation,
        category,
        difficulty || 5
      ...
      res.json(
        createResponse(true, learning, 'Vocabulary word added successfully')
      );
    } catch (error) {
      console.error('Error adding vocabulary:', error);
      res
        .status(500)
        .json(createResponse(false, null, 'Failed to add vocabulary word'));
    }
  })
);
...
router.post(
  '/language-learning/:id/pronunciation',
  upload.single('audio'),
  handleAsync(async (req, res) => {
    const { id } = req.params;
    ...
    try {
      ...
      const audioUrl = req.file
        ? `uploads/pronunciation/${req.file.originalname}`
        : null;
      ...
      res.json(
        createResponse(
          true,
          learning,
          'Pronunciation assessment submitted successfully'
        )
      );
    } catch (error) {
      ...
      res
        ...
        .json(
          createResponse(
            false,
            null,
            'Failed to submit pronunciation assessment'
          )
        );
    }
  })
...
router.put(
  ...
  handleAsync(async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json(createResponse(false, null, 'Failed to update progress'));
    }
  })
);
