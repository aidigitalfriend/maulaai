...
const communityContentSchema = new mongoose.Schema(
  {
    ...
    monetization: {
      ...
      revenueModel: {
        type: String,
        enum: ['subscription', 'one_time', 'donation', 'sponsored'],
      },
    },
...
communityContentSchema.virtual('engagementScore').get(function () {
  ...
  return (
    Math.round(((comments * 3 + reactions * 2 + shares * 5) / views) * 1000) /
    10
  );
});
...
// Static methods
communityContentSchema.statics.findPublished = function (
  groupId,
  options = {}
) {
  const query = {
...
communityContentSchema.statics.findByAuthor = function (
  authorId,
  groupId = null
) {
...
communityContentSchema.statics.searchContent = function (
  query,
  groupId = null,
  filters = {}
) {
...
communityContentSchema.statics.getContentStats = function (
  groupId,
  timeframe = 30
) {
...
communityContentSchema.methods.addView = function (
  userId = null,
  isUnique = false
) {
...
communityContentSchema.methods.addReaction = function (userId, reactionType) {
  ...
  this.analytics.totalEngagement =
    ...
    this.reactions.loves +
    this.reactions.laughs +
    this.comments.count +
...
communityContentSchema.methods.removeReaction = function (userId) {
  ...
  if (existingReaction) {
    // Decrease counter
    this.reactions[existingReaction.type] = Math.max(
      0,
      this.reactions[existingReaction.type] - 1
    );
    ...
    // Update total engagement
    this.analytics.totalEngagement = Math.max(
      0,
...
communityContentSchema.methods.addShare = function (shareType = 'internal') {
  ...
  this.analytics.shareBreakdown[shareType] =
    (this.analytics.shareBreakdown[shareType] || 0) + 1;
  this.analytics.totalEngagement += 1;
...
communityContentSchema.methods.updateQualityScore = function () {
  ...
  const engagementRate =
    this.analytics.views > 0
      ? (this.comments.count + this.reactions.likes) / this.analytics.views
      : 0;
...
communityContentSchema.methods.createVersion = function (
  newContent,
  editedBy,
  editReason
) {
...
communityContentSchema.pre('save', function (next) {
  ...
  if (
    this.isModified('analytics.views') ||
    this.isModified('comments.count') ||
  ...
  const hoursOld = this.publishedAt
    ? (new Date() - this.publishedAt) / (1000 * 60 * 60)
    : 999;
  if (hoursOld < 24 && this.metadata.engagement.score > 10) {
