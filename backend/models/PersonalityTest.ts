...
export interface IPersonalityTest extends mongoose.Document {
  ...
  title: string
  testType: 'big5' | 'mbti' | 'enneagram' | 'disc' | 'strengths' | 'emotional_intelligence' | 'career' | 'relationship' | 'custom'
  questions: {
    id: string
    question: string
...
const personalityTestSchema = new mongoose.Schema<IPersonalityTest>({
  ...
  testType: {
    type: String,
    enum: ['big5', 'mbti', 'enneagram', 'disc', 'strengths', 'emotional_intelligence', 'career', 'relationship', 'custom'],
    required: true,
    index: true
  },
  questions: [{
  ...
  results: {
    ...
    traits: [{
      ...
      category: { type: String, enum: ['strength', 'weakness', 'neutral'], required: true }
    }],
    dimensions: [{
  ...
  comparison: {
    previousTest: { type: mongoose.Schema.Types.ObjectId, ref: 'PersonalityTest' },
    changes: [{
...
personalityTestSchema.index({ 'sharing.shareCode': 1 })
...
const PersonalityTest = mongoose.models.PersonalityTest || mongoose.model<IPersonalityTest>('PersonalityTest', personalityTestSchema)
