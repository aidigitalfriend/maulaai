...
export interface ILanguageLearning extends mongoose.Document {
  ...
  proficiencyLevel: 'beginner' | 'elementary' | 'intermediate' | 'upper_intermediate' | 'advanced' | 'native'
  learningGoals: {
    primary: 'conversation' | 'business' | 'academic' | 'travel' | 'cultural' | 'exam_prep' | 'general'
    specific: string[]
    timeline: {
      target: string
      deadline: Date
  ...
  curriculum: {
    lessons: {
      id: string
      title: string
      type: 'vocabulary' | 'grammar' | 'pronunciation' | 'listening' | 'reading' | 'writing' | 'conversation'
      ...
      content: {
        ...
        exercises: {
          type: 'multiple_choice' | 'fill_blank' | 'translation' | 'pronunciation' | 'conversation' | 'listening'
          question: string
          options?: string[]
  ...
  aiTutor: {
    personalityType: 'encouraging' | 'strict' | 'patient' | 'funny' | 'professional'
  ...
  culturalLearning: {
    topics: {
      topic: string
      category: 'history' | 'customs' | 'food' | 'holidays' | 'social_norms' | 'arts' | 'politics'
      content: string
      media: {
...
const languageLearningSchema = new mongoose.Schema<ILanguageLearning>({
  ...
  learningGoals: {
    primary: {
      type: String,
      enum: ['conversation', 'business', 'academic', 'travel', 'cultural', 'exam_prep', 'general'],
      required: true
    },
  ...
  curriculum: {
    lessons: [{
      ...
      type: {
        type: String,
        enum: ['vocabulary', 'grammar', 'pronunciation', 'listening', 'reading', 'writing', 'conversation'],
        required: true
      },
      ...
      content: {
        ...
        materials: [{
          type: { type: String, enum: ['text', 'audio', 'video', 'interactive'], required: true },
        ...
        exercises: [{
          type: {
            type: String,
            enum: ['multiple_choice', 'fill_blank', 'translation', 'pronunciation', 'conversation', 'listening'],
            required: true
          },
  ...
  aiTutor: {
    ...
    interactions: [{
      ...
      type: { type: String, enum: ['lesson', 'practice', 'conversation', 'feedback', 'assessment'], required: true },
      messages: [{
  ...
  conversationPractice: {
    sessions: [{
      sessionId: { type: String, required: true },
      partner: { type: String, enum: ['ai', 'native_speaker', 'fellow_learner'], required: true },
  ...
  culturalLearning: {
    topics: [{
      ...
      category: {
        type: String,
        enum: ['history', 'customs', 'food', 'holidays', 'social_norms', 'arts', 'politics'],
        required: true
      },
      content: { type: String, required: true, maxlength: 2000 },
      media: [{
        type: { type: String, enum: ['image', 'video', 'article', 'podcast'], required: true },
  ...
  gamification: {
    ...
    streaks: [{
      type: { type: String, enum: ['daily_practice', 'lesson_completion', 'perfect_scores'], required: true },
...
languageLearningSchema.index({ proficiencyLevel: 1 })
...
languageLearningSchema.methods.addVocabularyWord = function(word: string, translation: string, category: string, difficulty: number) {
...
const LanguageLearning = mongoose.models.LanguageLearning || mongoose.model<ILanguageLearning>('LanguageLearning', languageLearningSchema)
