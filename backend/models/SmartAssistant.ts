...
const smartAssistantSchema = new mongoose.Schema<ISmartAssistant>(
  {
    ...
    personality: {
      ...
      communication: {
        ...
        style: {
          type: String,
          enum: ['direct', 'conversational', 'supportive', 'analytical'],
          required: true,
        },
      },
      behavior: {
      ...
      longTerm: [
        {
          category: {
            type: String,
            enum: ['preference', 'fact', 'relationship', 'goal', 'habit'],
            required: true,
          },
          key: { type: String, required: true, maxlength: 100 },
          value: mongoose.Schema.Types.Mixed,
      ...
      userFeedback: [
        {
          ...
          category: {
            type: String,
            enum: ['helpful', 'accurate', 'friendly', 'fast', 'understanding'],
            required: true,
          },
          timestamp: { type: Date, required: true },
        },
...
// Methods
smartAssistantSchema.methods.addMessage = function (
  conversationId: string,
  role: string,
  content: string,
  metadata?: any
...
smartAssistantSchema.methods.updateMemory = function (
  category: string,
  key: string,
  value: any,
  confidence: number = 0.8
) {
  const existingMemory = this.memory.longTerm.find(
    (mem) => mem.key === key && mem.category === category
  );
...
const SmartAssistant =
  mongoose.models.SmartAssistant ||
  mongoose.model<ISmartAssistant>('SmartAssistant', smartAssistantSchema);
