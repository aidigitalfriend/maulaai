...
class EmotionalTTSService {
  ...
  initializeKeys() {
    ...
    if (this.elevenLabsKey)
      console.log('\u2705 ElevenLabs TTS (PRIMARY - Emotional)');
  ...
  async speak(text, agentId, config) {
    ...
    const detectedEmotion =
      config?.emotion || EmotionDetector.detectEmotion(text);
    const detectedStyle =
      config?.style || EmotionDetector.determineSpeakingStyle(text, agentId);
    const intensity =
      config?.intensity || EmotionDetector.calculateIntensity(text);
    const finalConfig = {
    ...
    try {
      ...
      return await this.speakWithFallback(
        text,
        personality,
        finalConfig,
        provider,
  ...
  selectBestProvider(personality) {
    if (this.elevenLabsKey && personality.providers.elevenlabs)
      return 'elevenlabs';
  ...
  getAvailableProviders(agentId) {
    ...
    if (this.googleKey && personality.providers.google)
      available.push('google');
    if (this.awsAccessKey && personality.providers.polly)
      available.push('polly');
    if (this.openaiKey && personality.providers.openai)
  ...
  async testTTS(agentId) {
    ...
    const sampleTexts = {
      'julie-girlfriend':
        'Hey babe! I missed you so much today. How was your day?',
      'drama-queen':
        'Oh my goodness! This is ABSOLUTELY the most INCREDIBLE thing ever!',
      'emma-emotional':
      ...
      'professor-astrology':
        'The stars align in mysterious ways, revealing cosmic truths.',
      'travel-buddy':
        'Dude! This place is AMAZING! We gotta explore everything!',
      'ben-sega':
        'I can assist you with that request. Let me analyze the options.',
      'chess-player':
