...
export class MultiModalAIService {
    ...
    initializeClients() {
        ...
        if (this.nasaApiKey)
            console.log('✅ NASA API initialized');
        if (this.newsApiKey)
            console.log('✅ News API initialized');
        if (this.alphaVantageKey)
            console.log('✅ Alpha Vantage API initialized');
        if (this.shodanKey)
            console.log('✅ Shodan API initialized');
        if (this.pineconeKey)
            console.log('✅ Pinecone API initialized');
    ...
    async getChatResponse(message, agentId, config) {
        ...
        try {
            switch (provider) {
                ...
                case 'xai':
                    return await this.getChatXAI(message, systemPrompt, defaultConfig, startTime);
                default:
        ...
        for (const provider of providers) {
            ...
            try {
                switch (provider) {
                    case 'gemini':
                        if (this.gemini)
                            return await this.getChatGemini(message, systemPrompt, config, startTime);
                        break;
                    case 'openai':
                        if (this.openai)
                            return await this.getChatOpenAI(message, systemPrompt, config, startTime);
                        break;
                    case 'mistral':
                        if (this.mistral)
                            return await this.getChatMistral(message, systemPrompt, config, startTime);
                        break;
                    case 'anthropic':
                        if (this.anthropic)
                            return await this.getChatAnthropic(message, systemPrompt, config, startTime);
                        break;
                    case 'xai':
                        if (this.xaiApiKey)
                            return await this.getChatXAI(message, systemPrompt, config, startTime);
                        break;
                    case 'cohere':
                        if (this.cohere)
                            return await this.getChatCohere(message, systemPrompt, config, startTime);
                        break;
                }
            }
            catch (error) {
    ...
    async getChatXAI(message, systemPrompt, config, startTime) {
        if (!this.xaiApiKey)
            throw new Error('xAI not initialized');
    ...
    async getNASAData(endpoint = 'planetary/apod') {
        ...
        const response = await fetch(`https://api.nasa.gov/${endpoint}?api_key=${this.nasaApiKey}`);
    ...
    async getNews(query, language = 'en') {
        ...
        const response = await fetch(`https://newsapi.org/v2/everything?q=${encodeURIComponent(query)}&language=${language}&apiKey=${this.newsApiKey}`);
        return await response.json();
    }
    /**
    ...
    async getStockData(symbol, interval = 'daily') {
        ...
        const response = await fetch(`https://www.alphavantage.co/query?function=${functionMap[interval] || functionMap.daily}&symbol=${symbol}&apikey=${this.alphaVantageKey}`);
        return await response.json();
    }
    /**
    ...
    async getShodanData(query) {
        ...
        const response = await fetch(`https://api.shodan.io/shodan/host/search?key=${this.shodanKey}&query=${encodeURIComponent(query)}`);
        return await response.json();
    }
    /**
