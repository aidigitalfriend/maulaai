 ...
 */
export function getAgentPersonalityConfig(agentId) {
 ...
 */
export function buildAgentSystemMessage(agentId, additionalContext) {
 ...
 */
export function preparePersonalizedRequest(request) {
 ...
 */
export function validatePersonalityMaintenance(agentId, response) {
    const warnings = [];
    const suggestions = [];
    ...
    if ((agentId === 'comedy-king' || agentId === 'drama-queen') &&
        response.length < 80) {
 ...
 */
export function getOptimalTemperature(agentId) {
    return AGENT_TEMPERATURES[agentId] || 0.7;
}
/**
 * Create a personality-enforced API payload
 */
export function createPersonalityEnforcedPayload(request) {
