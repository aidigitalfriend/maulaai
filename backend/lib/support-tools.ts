...
const mockKnowledgeBase: KnowledgeBaseResult[] = [
  ...
  },
  {
    ...
    content:
      'Billing cycles: Choose $1 daily, $5 weekly, or $19 monthly per agent. Each subscription covers exactly one AI agent and renews based on the interval you pick.',
    category: 'Billing',
    relevanceScore: 0.9,
    url: '/pricing',
  },
  {
    ...
    content:
      'To integrate with external APIs: 1. Go to Settings > Integrations 2. Add your API keys 3. Test the connection 4. Enable the integration',
...
export class SupportTools {
  ...
  static async createTicket(payload: TicketPayload): Promise<Ticket> {
    try {
      ...
      const slaHours =
        payload.severity === 'P1' ? 4 : payload.severity === 'P2' ? 24 : 48;
   ...
   */
  private static async sendTicketNotification(
    ticket: Ticket,
