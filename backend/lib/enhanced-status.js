...
async function getEnhancedStatus(fallbackMetrics, providers, dbStatus) {
  try {
    ...
    const agentsData = agentMetrics.length > 0 ? agentMetrics.map((agent) => ({
    ...
    const historicalData = historical.length >= 7 ? historical.slice(-7) : Array.from({ length: 7 }, (_, i) => {
      const d = new Date(now);
      d.setDate(now.getDate() - (6 - i));
      return {
        date: d.toISOString(),
        uptime: platformStatus === "operational" ? 99.99 : 98.5,
        requests: 5e3 + i * 421 + Math.floor(Math.random() * 500),
...
async function getEnhancedAnalytics() {
  try {
    ...
    const agentsData = agentMetrics.length > 0 ? agentMetrics.map((agent) => ({
      ...
      successRate: agent.requestCount > 0 ? Math.round(agent.successCount / agent.requestCount * 100 * 10) / 10 : 100,
    ...
    const topAgents = agentsData.sort((a, b) => b.requests - a.requests).slice(0, 3).map((agent) => ({
      name: agent.name,
...
async function getEnhancedApiStatus() {
  try {
    ...
    return {
      endpoints: [
        { name: "Health", endpoint: "/health", method: "GET", status: "operational", responseTime: 1, uptime: 99.9, lastChecked: (/* @__PURE__ */ new Date()).toISOString(), errorRate: 0 },
        { name: "Chat", endpoint: "/api/chat", method: "POST", status: apiStats.errorRate < 1 ? "operational" : "degraded", responseTime: apiStats.avgResponseTime, uptime: apiStats.successRate, lastChecked: (/* @__PURE__ */ new Date()).toISOString(), errorRate: apiStats.errorRate },
        { name: "Language Detect", endpoint: "/api/language-detect", method: "POST", status: "operational", responseTime: apiStats.avgResponseTime, uptime: 99.9, lastChecked: (/* @__PURE__ */ new Date()).toISOString(), errorRate: 0 },
        { name: "Translate", endpoint: "/api/translate", method: "POST", status: "operational", responseTime: apiStats.avgResponseTime, uptime: 99.9, lastChecked: (/* @__PURE__ */ new Date()).toISOString(), errorRate: 0 }
      ],
      categories: {
