...
async function speakElevenLabs(text, personality, config, apiKey) {
...
function mapEmotionToElevenLabs(emotion, intensity, personality) {
  ...
  case 'cheerful':
    ...
    similarity = Math.min(1, baseSettings.similarityBoost + 0.1);
    style = 0.7 + intensity * 0.3;
    break;
  case 'sad':
  case 'melancholic':
  case 'disappointed':
    stability = Math.min(0.8, baseSettings.stability + 0.2);
  ...
  case 'frustrated':
  case 'annoyed':
    stability = Math.max(0.2, baseSettings.stability - 0.3);
...
async function speakAzure(text, personality, config, apiKey, region) {
...
function buildAzureSSML(text, voiceName, config, _personality) {
  const azureStyle = mapEmotionToAzureStyle(config.emotion);
  const styleDegree = config.intensity.toFixed(2);
  const rate = config.speed === 1 ? 'medium' : `${(config.speed * 100).toFixed(0)}%`;
  const pitch = config.pitch === 0 ? 'medium' : `${config.pitch > 0 ? '+' : ''}${config.pitch}st`;
  const volume = config.volume === 1 ? 'default' : `${(config.volume * 100).toFixed(0)}%`;
  return `
...
function mapEmotionToAzureStyle(emotion) {
  const mapping = {
    'neutral': 'general',
    'happy': 'cheerful',
    'joyful': 'cheerful',
...
async function speakGoogle(text, personality, config, apiKey) {
...
function mapEmotionToGoogleEffects(config, _personality) {
  const baseConfig = {
    audioEncoding: 'MP3',
    speakingRate: config.speed,
    pitch: config.pitch,
    volumeGainDb: (config.volume - 1) * 16,
    // Convert to dB
  };
  const effects = [];
...
async function speakPolly(text, personality, config, accessKey, secretKey) {
...
function buildPollySSML(text, config, _personality) {
  const rate = `${(config.speed * 100).toFixed(0)}%`;
  const pitch = `${config.pitch > 0 ? '+' : ''}${config.pitch}%`;
  const volume = config.volume === 1 ? 'medium' : `${(config.volume * 100).toFixed(0)}%`;
...
async function speakOpenAI(text, personality, config, apiKey) {
...
function calculateCost(provider, textLength) {
  const charCount = textLength;
  switch (provider) {
  case 'elevenlabs':
    return charCount / 1e3 * 0.3;
    // $0.30 per 1K chars
  case 'azure':
    return charCount / 1e6 * 16;
    // $16 per 1M chars
  case 'google':
    return charCount / 1e6 * 16;
    // $16 per 1M chars
  ...
  case 'openai':
    return charCount / 1e3 * 0.015;
    // $15 per 1M chars = $0.015 per 1K
  default:
    return 0;
  }
}
