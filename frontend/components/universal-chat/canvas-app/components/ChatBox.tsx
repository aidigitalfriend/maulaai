...
const ChatBox: React.FC<ChatBoxProps> = ({ messages, onSendMessage, isGenerating, onNewChat }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-[#0a0a0a] w-full relative">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-800/50 shrink-0">
        <div className="flex items-center gap-2">
          <button 
            ...
            className={`p-2 rounded-lg transition-all ${showChatHistory ? 'text-cyan-400 bg-cyan-500/10' : 'text-gray-500 hover:text-cyan-400 hover:bg-cyan-500/10'}`}
            title="Chat History"
          >
        ...
        <button 
          ...
          className="flex items-center gap-1 px-3 py-1.5 text-[10px] font-bold text-cyan-400 hover:text-white bg-cyan-500/10 hover:bg-cyan-500/20 border border-cyan-500/30 rounded-lg transition-all uppercase tracking-widest"
        >
      ...
      {showChatHistory && (
        <div className="absolute inset-0 top-[52px] bg-[#0a0a0a] z-10 flex flex-col">
