...
const App: React.FC = () => {
  ...
  // App Data State - SSR safe (check for window before using localStorage)
  const [sessions, setSessions] = useState<ChatSession[]>(defaultSessions);

  // Load from localStorage on client-side mount, and fix initial timestamp
  useEffect(() => {
  ...
  const handleSend = async (text: string) => {
    ...
    // Call secure backend API (supports all 7 providers)
    const result = await sendMessage(text, activeSession.settings, history);
    setIsThinking(false);

    // Log provider used for debugging
    console.log(`[Neural-Link] Response from: ${result.provider} (${result.durationMs}ms)`);
