...
export default function UniversalAgentChat({ agent }: UniversalAgentChatProps) {
  ...
  const handleExportSession = useCallback(
    (id: string) => {
      ...
      a.href = url;
      a.download = `${agent.name.replace(/[^a-z0-9]/gi, '_')}_${session.name.replace(/[^a-z0-9]/gi, '_')}_${Date.now()}.txt`;
      a.click();
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    const agentConfig: AgentConfig = {
      ...
      provider: settings.provider === 'openai' ? 'mistral' : (settings.provider as 'mistral' | 'gemini'),
    };
                    ...
                    messages: s.messages.map((m) =>
                      m.id === assistantMessageId
                        ? {
                            ...
                            content: fullMessage,
                            isStreaming: false,
                            codeBlocks: detectedCodeBlocks.length > 0 ? detectedCodeBlocks : undefined,
                          }
                        : m
                    ),
                  }
    ...
    >
      <div className="flex flex-col h-full">
              ...
              >
                <div className="whitespace-pre-wrap text-sm leading-relaxed">
                  {message.content.split('\n').map((line, i) => {
                    ...
                    if (line.startsWith('## ')) {
                      return <h3 key={`line-${i}`} className="font-bold text-base mt-2">{line.slice(3)}</h3>;
                    }
                    if (line.startsWith('# ')) {
                      return <h2 key={`line-${i}`} className="font-bold text-lg mt-2">{line.slice(2)}</h2>;
                    }
                    if (line.startsWith('> ')) {
                      return (
                        <blockquote key={`line-${i}`} className="border-l-4 border-indigo-300 pl-3 italic my-2 text-gray-600">
                    ...
                    return <span key={`line-${i}`} dangerouslySetInnerHTML={{ __html: processedLine }} />;
                  }).filter(Boolean)}
                </div>
        ...
        <div className="flex-shrink-0 px-4 py-2 border-t border-gray-200 bg-white/80 backdrop-blur-sm">
          <input ref={fileInputRef} type="file" className="hidden" aria-label="File upload"
            onChange={(e) => {
