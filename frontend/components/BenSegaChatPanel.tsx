// @/frontend/components/BenSegaChatPanel.tsx
"use client";
...
interface BenSegaChatPanelProps {
...
export default function BenSegaChatPanel({
  chatSessions,
  activeSessionId,
  onNewChat,
  onSelectChat,
  ...
  const handleExportChat = (session: ChatSession) => {
    try {
      ...
      const agentHistory = histories['ben-sega'];
  ...
  const handleShareChat = (session: ChatSession) => {
    ...
    const shareUrl = `${window.location.origin}/agents/ben-sega?session=${session.id}`;
