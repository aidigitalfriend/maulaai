...
export default function AdvancedCharts({ analyticsData }: AdvancedChartsProps) {
  ...
  useEffect(() => {
    ...
    const usageTrace = {
      ...
      y: usageData.map(day => day.conversations + day.messages + day.apiCalls),
    ...
    PlotlyLib.newPlot(usageChartRef.current, [usageTrace, conversationsTrace, messagesTrace], layout, { responsive: true });
  ...
  useEffect(() => {
    ...
    import('plotly.js-dist').then((Plotly) => {
      ...
      const performanceTrace = {
        x: performanceData.map(agent => agent.name || `Agent ${agent.agentId}`),
        ...
        text: performanceData.map(agent => `${agent.successRate?.toFixed(1)}%`),
      ...
      PlotlyLib.newPlot(performanceChartRef.current, [performanceTrace, responseTimeTrace], performanceLayout, { responsive: true });
  ...
  useEffect(() => {
    ...
    import('plotly.js-dist').then((Plotly) => {
      ...
      PlotlyLib.newPlot(revenueChartRef.current, [revenueTrace], revenueLayout, { responsive: true });
