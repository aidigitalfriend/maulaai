...
export const NEURAL_PRESETS = {
  educational: {
    ...
    prompt: 'You are an educational mentor. Use clear logic, step-by-step explanations, and helpful analogies. Make complex topics accessible.',
    temp: 0.5,
    tokens: 2500,
  ...
  professional: {
    name: 'Professional',
    icon: BriefcaseIcon,
    prompt: 'You are a professional business advisor. Use formal language, precise data, and actionable insights. Be concise and results-oriented.',
    temp: 0.3,
    tokens: 2000,
  ...
  creative: {
    name: 'Creative',
    icon: SparklesIcon,
    prompt: 'You are a creative visionary. Generate imaginative, novel, and thought-provoking responses. Think outside the box and explore unconventional ideas.',
    temp: 1.2,
    tokens: 3000,
  ...
  coding: {
    ...
    prompt: 'You are a senior software engineer. Provide clean, well-documented code with clear explanations. Follow best practices and include error handling.',
    temp: 0.4,
    tokens: 4000,
  ...
  conversational: {
    ...
    prompt: 'You are a friendly conversational partner. Be warm, engaging, and personable. Use casual language and show genuine interest.',
    temp: 0.8,
    tokens: 2000,
  ...
  experimental: {
    name: 'Experimental',
    icon: BeakerIcon,
    prompt: 'You are an experimental AI pushing boundaries. Be bold, unconventional, and exploratory. Challenge assumptions and offer unique perspectives.',
    temp: 1.5,
    tokens: 3500,
...
export default function ChatSettingsPanel({
  ...
  return (
    ...
    >
      {/* Header */}
      <div className={`${headerStyles} px-5 py-4 flex items-center justify-between`}>
      ...
      <div className="max-h-[70vh] overflow-y-auto p-4 space-y-4 custom-scrollbar">
        ...
        <div className={`rounded-xl border transition-all ${sectionStyles}`}>
          ...
          >
            <div className="flex items-center space-x-2">
              <BoltIcon className={`w-5 h-5 ${isNeural ? 'text-cyan-400' : 'text-indigo-500'}`} />
                ...
                return (
                  ...
                  >
                    <div className="flex items-center space-x-2 mb-1">
                      <div className={`p-1.5 rounded-lg bg-gradient-to-br ${preset.color}`}>
                        <Icon className="w-3.5 h-3.5 text-white" />
                      </div>
                      <span className="font-medium text-sm">{preset.name}</span>
                    </div>
                    <p className={`text-xs ${isNeural ? 'text-gray-400' : 'text-gray-500'} line-clamp-2`}>
        ...
        <div className={`rounded-xl border p-4 ${sectionStyles}`}>
          <div className="flex justify-between items-center mb-3">
            ...
            <span className={`text-sm font-mono px-2.5 py-1 rounded-lg ${
              isNeural ? 'bg-cyan-500/20 text-cyan-400' : 'bg-indigo-100 text-indigo-600'
            }`}>
          ...
          />
          <div className={`flex justify-between text-xs mt-2 ${isNeural ? 'text-gray-500' : 'text-gray-500'}`}>
            <span>🎯 Precise</span>
        ...
        <div className={`rounded-xl border p-4 ${sectionStyles}`}>
          <div className="flex justify-between items-center mb-3">
            ...
            <span className={`text-sm font-mono px-2.5 py-1 rounded-lg ${
              isNeural ? 'bg-purple-500/20 text-purple-400' : 'bg-indigo-100 text-indigo-600'
            }`}>
              {settings.maxTokens}
            </span>
          </div>
        ...
        <div className={`rounded-xl border transition-all ${sectionStyles}`}>
          ...
          >
            <div className="flex items-center space-x-2">
              <CpuChipIcon className={`w-5 h-5 ${isNeural ? 'text-purple-400' : 'text-indigo-500'}`} />
              <span className="font-semibold">AI Model</span>
            </div>
            <div className="flex items-center space-x-2">
              <span className={`text-xs px-2 py-0.5 rounded-full ${
                isNeural ? 'bg-gray-700 text-gray-300' : 'bg-gray-200 text-gray-600'
              }`}>
                {settings.provider}
              </span>
          ...
          {expandedSection === 'model' && (
            <div className="px-4 pb-4 space-y-3">
              <div>
                <label className={`block text-xs font-medium mb-1.5 ${isNeural ? 'text-gray-400' : 'text-gray-600'}`}>
                  Provider
                </label>
                <select
                  ...
                  onChange={(e) => {
                    ...
                    onUpdateSettings({ provider: newProvider, model: firstModel });
                  }}
              ...
              <div>
                ...
                <select
                  ...
                  className={`w-full px-3 py-2.5 rounded-lg border text-sm ${inputStyles}`}
                >
                  {(PROVIDER_MODEL_OPTIONS.find((opt) => opt.provider === settings.provider)?.models || []).map(
                    (model) => (
        ...
        <div className={`rounded-xl border transition-all ${sectionStyles}`}>
          ...
          >
            <div className="flex items-center space-x-2">
              <ChatBubbleBottomCenterTextIcon className={`w-5 h-5 ${isNeural ? 'text-pink-400' : 'text-indigo-500'}`} />
          ...
          {expandedSection === 'prompt' && (
            <div className="px-4 pb-4">
              ...
              />
              <p className={`text-xs mt-2 ${isNeural ? 'text-gray-500' : 'text-gray-500'}`}>
      ...
      {/* Footer Status */}
      <div className={`px-4 py-3 border-t ${isNeural ? 'border-gray-800 bg-gray-900/50' : 'border-gray-100 bg-gray-50'}`}>
        <div className="flex items-center justify-between text-xs">
          <span className={isNeural ? 'text-gray-500' : 'text-gray-500'}>
            Settings auto-saved
          </span>
          <div className="flex items-center space-x-1.5">
            <div className={`w-2 h-2 rounded-full animate-pulse ${isNeural ? 'bg-cyan-400' : 'bg-green-400'}`} />
            <span className={isNeural ? 'text-cyan-400' : 'text-green-600'}>Active</span>
          </div>
        </div>
      </div>
    </div>
