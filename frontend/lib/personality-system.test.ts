...
export class PersonalitySystemTests {
  ...
  static testPromptStructure(): boolean {
    ...
    TEST_AGENTS.forEach((agentId) => {
      ...
      if (missingSection) {
        console.error(
          `❌ Agent "${agentId}" missing section: "${missingSection}"`
        );
        passed = false;
      }
    });
  ...
  static testNoGenericLanguage(): boolean {
    ...
    TEST_AGENTS.forEach((agentId) => {
      const prompt = getSystemPrompt(agentId);
      const foundGeneric = genericPatterns.find((pattern) =>
        pattern.test(prompt)
      );

      if (foundGeneric) {
        console.error(
          `❌ Agent "${agentId}" contains generic language: "${foundGeneric}"`
        );
        passed = false;
      }
    });
  ...
  static testPersonalitySpecificity(): boolean {
    ...
    Object.entries(personalityChecks).forEach(([agentId, { must }]) => {
      const prompt = getSystemPrompt(agentId);
      const missingKeywords = must.filter(
        (keyword) => !prompt.toLowerCase().includes(keyword.toLowerCase())
      );

      if (missingKeywords.length > 0) {
        console.error(
          `❌ Agent "${agentId}" missing keywords: ${missingKeywords.join(
            ', '
          )}`
        );
        passed = false;
    ...
    if (passed)
      console.log('✅ All agents have specific personality indicators');
    return passed;
  }
  ...
  static testNeverAlwaysRules(): boolean {
    ...
    if (passed)
      console.log('✅ All agents have NEVER/ALWAYS enforcement rules');
    return passed;
  }
