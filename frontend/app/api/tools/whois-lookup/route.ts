...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(
      'WHOIS API Request URL:',
      apiUrl.replace(whoisApiKey, whoisApiKey.substring(0, 10) + '...')
    );
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `WHOIS API request failed: ${response.status} ${response.statusText}`
      );
    }
    ...
    if (data.ErrorMessage) {
      ...
      if (
        ...
        errorMsg.includes('authenticate failed')
      ) {
        return NextResponse.json(
          {
            success: false,
            error:
              'WHOIS API service is currently unavailable. The API key may be invalid or has exceeded its quota. Please try again later or contact support.',
          },
          {
            status: 503,
            headers: {
    ...
    const result = {
      domain: cleanDomain,
      registrar:
        registryData.registrarName || whoisRecord.registrarName || 'N/A',
      ...
      registrantCountry:
        registryData.registrant?.country || whoisRecord.registrant?.country,
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            'WHOIS service is currently unavailable. The API key may be invalid or the whois command is not installed on the server.',
        },
        {
          status: 503,
          headers: {
