...
export async function POST(request: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            'Domain reputation service is experiencing high demand. Please try again shortly. 🙏',
        },
        { status: 429 },
        {
          headers: {
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            'Domain reputation service is temporarily unavailable. Please try again later. 😊',
        },
        { status: 503 },
        {
          headers: {
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            'Unable to check domain reputation. Please verify the domain and try again. 🌐',
        },
        { status: 400 },
        {
          headers: {
    ...
    if (reputationScore >= 80) {
      ...
      verdict =
        'This domain has an excellent reputation and is highly trustworthy! ✓';
