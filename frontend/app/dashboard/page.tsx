      ...
      <div className="container-custom section-padding-lg">
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-neural-200 min-w-[280px]">
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <p className="text-2xl font-bold text-neural-800">{analyticsData.subscription.plan}</p>
              </div>
            ...
            <div className="flex items-baseline gap-1 mb-2">
              <span className="text-3xl font-bold text-brand-600">${analyticsData.subscription.price}</span>
              <span className="text-neural-600">/{analyticsData.subscription.period}</span>
            </div>
            <p className="text-sm text-neural-600">
              Renews in <span className="font-semibold text-neural-800">{analyticsData.subscription.daysUntilRenewal} days</span>
            </p>
            <Link href="/pricing/overview" className="mt-4 w-full btn-secondary text-center block text-sm">
              Upgrade Plan
            </Link>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          {quickStats.map((stat, index) => {
            ...
            return (
              <div key={index} className="bg-white rounded-xl p-6 shadow-sm border border-neural-100 hover:shadow-md transition-shadow">
                ...
                <h3 className="text-sm font-medium text-neural-600 mb-1">{stat.label}</h3>
                <div className="text-3xl font-bold text-neural-800 mb-2">{stat.value}</div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
          ...
          <div className="bg-white rounded-xl p-8 shadow-sm border border-neural-100">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-neural-800">Resource Usage</h2>
              ...
              {Object.entries(analyticsData.usage).map(([key, value]) => (
                <div key={key}>
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className={`text-sm font-semibold ${getStatusColor(value.percentage)}`}>
                      {value.current.toLocaleString()} / {value.limit.toLocaleString()} {value.unit || ''}
                    </span>
                  </div>
                  <div className="w-full bg-neural-200 rounded-full h-3 overflow-hidden">
                    <div 
                      className={`h-3 rounded-full transition-all duration-500 ${getProgressColor(value.percentage)}`}
                      style={{ width: `${Math.min(value.percentage, 100)}%` }}
                    ></div>
                  </div>
                  <p className="text-xs text-neural-500 mt-1">{value.percentage.toFixed(1)}% used</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl p-8 shadow-sm border border-neural-100">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-neural-800">7-Day Activity</h2>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-end justify-between h-48 gap-2">
                {analyticsData.dailyUsage.map((day, index) => {
                  const maxConversations = Math.max(...analyticsData.dailyUsage.map(d => d.conversations))
                      ...
                      <div className="relative w-full">
                        ...
                        >
                          ...
                          <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 opacity-0 group-hover:opacity-100 transition-opacity bg-neural-900 text-white text-xs rounded-lg px-3 py-2 whitespace-nowrap z-10">
                            <p className="font-semibold">{day.conversations} conversations</p>
              ...
              <div className="flex items-center gap-6 pt-4 border-t border-neural-100">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-neural-600">Conversations</span>
                </div>
                <div className="flex items-center gap-2 text-sm">
                  <TrendingUp className="w-4 h-4 text-green-600" />
                  <span className="text-green-600 font-semibold">{analyticsData.weeklyTrend.conversationsChange}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
          ...
          <div className="bg-white rounded-xl p-8 shadow-sm border border-neural-100">
            <h2 className="text-2xl font-bold text-neural-800 mb-6">Top Agents</h2>
            <div className="space-y-4">
              {analyticsData.topAgents.map((agent, index) => (
                <div key={index}>
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-sm font-medium text-neural-700">{agent.name}</span>
                    <span className="text-sm font-semibold text-brand-600">{agent.usage}%</span>
                  </div>
          ...
          <div className="bg-white rounded-xl p-8 shadow-sm border border-neural-100">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-neural-800">Cost Analysis</h2>
              <DollarSign className="w-6 h-6 text-green-600" />
            </div>
            <div className="space-y-4 mb-6">
              <div className="flex justify-between items-center p-4 bg-blue-50 rounded-lg">
                <span className="text-sm text-neural-700">Current Month</span>
                <span className="text-2xl font-bold text-brand-600">${analyticsData.costAnalysis.currentMonth}</span>
              </div>
              <div className="flex justify-between items-center p-4 bg-purple-50 rounded-lg">
                <span className="text-sm text-neural-700">Projected Total</span>
                <span className="text-2xl font-bold text-purple-600">${analyticsData.costAnalysis.projectedMonth}</span>
              </div>
            </div>
            <div className="space-y-3">
              <p className="text-sm font-semibold text-neural-700 mb-3">Cost Breakdown</p>
              {analyticsData.costAnalysis.breakdown.map((item, index) => (
                <div key={index} className="flex justify-between items-center">
                  <span className="text-sm text-neural-600">{item.category}</span>
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-semibold text-neural-800">${item.cost}</span>
                    <span className="text-xs text-neural-500">({item.percentage}%)</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-white rounded-xl p-8 shadow-sm border border-neural-100 mb-12">
          <h2 className="text-2xl font-bold text-neural-800 mb-6">Agent Performance Details</h2>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-neural-200">
                  <th className="text-left py-3 px-4 text-sm font-semibold text-neural-700">Agent Name</th>
                  <th className="text-left py-3 px-4 text-sm font-semibold text-neural-700">Conversations</th>
                  <th className="text-left py-3 px-4 text-sm font-semibold text-neural-700">Messages</th>
                  <th className="text-left py-3 px-4 text-sm font-semibold text-neural-700">Avg Response</th>
                  <th className="text-left py-3 px-4 text-sm font-semibold text-neural-700">Success Rate</th>
                </tr>
              </thead>
              <tbody>
                {analyticsData.agentPerformance.map((agent, index) => (
                  <tr key={index} className="border-b border-neural-100 hover:bg-neural-50 transition-colors">
                    <td className="py-4 px-4">
                      <span className="font-medium text-neural-800">{agent.name}</span>
                    </td>
                    <td className="py-4 px-4 text-neural-700">{agent.conversations.toLocaleString()}</td>
                    <td className="py-4 px-4 text-neural-700">{agent.messages.toLocaleString()}</td>
                    <td className="py-4 px-4">
        ...
        <div className="bg-white rounded-xl p-8 shadow-sm border border-neural-100 mb-12">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl font-bold text-neural-800">Recent Activity</h2>
            <span className="text-sm text-neural-600">Last 30 minutes</span>
          </div>
          <div className="space-y-3">
            {analyticsData.recentActivity.map((activity, index) => (
              <div key={index} className="flex items-center justify-between p-4 bg-neural-50 rounded-lg hover:bg-neural-100 transition-colors">
                <div className="flex items-center gap-4">
                  <div className={`w-2 h-2 rounded-full ${
                    activity.status === 'success' ? 'bg-green-500' : 
                    activity.status === 'active' ? 'bg-blue-500 animate-pulse' : 'bg-red-500'
                  }`}></div>
                  <div>
                    <p className="text-sm font-medium text-neural-800">{activity.action}</p>
                    <p className="text-xs text-neural-600">{activity.agent}</p>
                  </div>
                </div>
                <span className="text-xs text-neural-500">{activity.timestamp}</span>
              </div>
            ))}
          </div>
        </div>
