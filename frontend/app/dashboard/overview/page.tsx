...
const generateMockSecuritySettings = () => ({
  ...
  trustedDevices: [
    { name: 'MacBook Pro', lastUsed: '2025-11-26', location: 'San Francisco, CA' },
    { name: 'iPhone 15', lastUsed: '2025-11-25', location: 'San Francisco, CA' }
  ],
  recentLogins: [
    { timestamp: '2025-11-26 09:30 AM', location: 'San Francisco, CA', success: true },
    { timestamp: '2025-11-25 08:45 PM', location: 'San Francisco, CA', success: true },
    { timestamp: '2025-11-25 02:15 PM', location: 'San Francisco, CA', success: true }
  ]
})
...
const generateMockRewards = () => ({
  ...
  badges: [
    { name: 'Early Adopter', description: 'One of our first users', icon: '🌟', category: 'Milestone' },
    { name: 'AI Expert', description: 'Completed 100+ AI tasks', icon: '🤖', category: 'Achievement' },
    { name: 'Community Helper', description: 'Helped 50+ users', icon: '🤝', category: 'Social' }
  ],
  streaks: {
    current: 15,
    longest: 28
  ...
  achievements: [
    { name: 'Daily User', progress: 15, maxProgress: 30, description: 'Use platform daily for 30 days' },
    { name: 'Feature Explorer', progress: 8, maxProgress: 10, description: 'Try 10 different features' }
  ]
})
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="section-padding bg-gradient-to-r from-brand-600 to-accent-600 text-white">
        <div className="container-custom text-center">
          ...
          <p className="text-xl opacity-90 mb-8">Get started with your analytics and insights</p>
          
          {/* Go to Dashboard button in hero section */}
          <Link href="/dashboard" className="inline-flex items-center bg-white text-brand-600 px-8 py-3 rounded-lg font-semibold hover:bg-neural-50 transition-colors">
            Go to Dashboard
          </Link>
        </div>
      </section>
            ...
            <Link href="/dashboard/profile" className="group">
              <div className="p-8 border-2 border-neural-200 rounded-lg hover:border-brand-300 hover:shadow-lg transition-all">
                <div className="flex items-center mb-6">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-neural-900">User Profile</h3>
                    <p className="text-neural-600">Manage your personal information</p>
                  </div>
                </div>
                ...
                {userProfile && (
                  <div className="space-y-3">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <p className="font-medium text-neural-900">{userProfile.name}</p>
                        <p className="text-sm text-neural-600">{userProfile.email}</p>
                      </div>
                    </div>
                    <div className="text-sm text-neural-700 space-y-1">
                      <p>📍 {userProfile.location}</p>
                      <p>💼 {userProfile.profession} at {userProfile.company}</p>
            ...
            <Link href="/dashboard/security" className="group">
              <div className="p-8 border-2 border-neural-200 rounded-lg hover:border-brand-300 hover:shadow-lg transition-all">
                <div className="flex items-center mb-6">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-neural-900">Security Settings</h3>
                    <p className="text-neural-600">Password, 2FA, and security options</p>
                  </div>
                </div>
                ...
                {securitySettings && (
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-neural-700">Two-Factor Authentication</span>
                    ...
                    <div className="text-sm text-neural-700">
                      <p>🔐 Password last changed: {new Date(securitySettings.passwordLastChanged).toLocaleDateString()}</p>
                      <p>📱 {securitySettings.trustedDevices.length} trusted devices</p>
                    </div>
                  </div>
                )}
            ...
            <Link href="/dashboard/preferences" className="group">
              <div className="p-8 border-2 border-neural-200 rounded-lg hover:border-brand-300 hover:shadow-lg transition-all">
                <div className="flex items-center mb-6">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-neural-900">Preferences</h3>
                    <p className="text-neural-600">Themes, languages, and settings</p>
                  </div>
                </div>
            ...
            <Link href="/dashboard/rewards" className="group">
              <div className="p-8 border-2 border-neural-200 rounded-lg hover:border-brand-300 hover:shadow-lg transition-all relative overflow-hidden">
                ...
                <div className="flex items-center mb-6">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-neural-900">Rewards Center</h3>
                    <p className="text-neural-600">Points, badges, and achievements</p>
                  </div>
                </div>
                ...
                {rewards && (
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-2xl font-bold text-brand-600">{rewards?.totalPoints?.toLocaleString() || '0'}</p>
                        <p className="text-sm text-neural-600">Total Points</p>
                      </div>
                      <div className="text-right">
                        <div className="flex items-center space-x-1">
                          <StarIcon className="w-5 h-5 text-yellow-500" />
                          <span className="text-lg font-semibold">Level {rewards?.currentLevel || 1}</span>
                        </div>
                        <p className="text-xs text-neural-600">{rewards?.pointsToNextLevel || 0} to next level</p>
                      </div>
                    </div>
                    
                    <div className="grid grid-cols-3 gap-2">
                      {rewards?.badges?.slice(0, 3).map((badge: any, idx: number) => (
                        <div key={idx} className="text-center">
                          <div className="text-2xl mb-1">{badge.icon || '🎖️'}</div>
                          <p className="text-xs text-neural-600 truncate">{badge.name}</p>
                        </div>
                      )) || (
                    ...
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-neural-700">🔥 {rewards?.streaks?.current || 0} day streak</span>
                      <span className="text-neural-700">🏆 {rewards?.badges?.length || 0} badges earned</span>
                    </div>
                  </div>
                )}
      ...
      <section className="section-padding bg-neural-50">
        <div className="container-custom max-w-4xl">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-neural-900 mb-4">Your Activity Overview</h2>
            <p className="text-lg text-neural-600">Track your progress and engagement</p>
          </div>
